...
try {
    $stmt = $conn->query("
        SELECT s.student_id, s.name AS student_name, s.bus_id, b.bus_number AS bus_number
        FROM students s
...
try {
    $stmt = $conn->query("SELECT bus_id, bus_number FROM buses ORDER BY bus_number ASC");
...
<style>
body { margin: 0; font-family: "Segoe UI", sans-serif; background: linear-gradient(135deg, #dce35b, #45b649); display: flex; justify-content: center; align-items: flex-start; min-height: 100vh; padding: 30px; }
.card { width: 100%; max-width: 1100px; background: white; padding: 25px; border-radius: 16px; box-shadow: 0 8px 25px rgba(0,0,0,0.1); animation: fadeIn 0.4s ease-in-out; }
...
.table-container { overflow-x: auto; max-height: 65vh; border-radius: 10px; box-shadow: 0 4px 15px rgba(0,0,0,0.05); }
table { width: 100%; border-collapse: collapse; min-width: 700px; }
th, td { padding: 12px; text-align: center; font-size: 0.95rem; }
th { background: #f9fafb; font-weight: bold; border-bottom: 2px solid #e5e7eb; position: sticky; top: 0; z-index: 2; }
tr:nth-child(even) td { background: #fcfcfc; }
tr:hover td { background: #f1f5f9; transition: 0.2s; }
select { padding: 6px; border-radius: 6px; border: 1px solid #ccc; }
button { background: #007bff; color: white; border: none; border-radius: 6px; padding: 6px 10px; cursor: pointer; transition: background 0.3s, transform 0.2s; }
...
<td><?= htmlspecialchars($student['bus_number']) ?? "<em>Not assigned</em>" ?></td>
<td>
...
<option value="<?= $bus['bus_id'] ?>" <?= $student['bus_id'] == $bus['bus_id'] ? 'selected' : '' ?>>
